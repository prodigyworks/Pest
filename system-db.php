		...
		if (! isset($_SESSION['SITE_CONFIG'])) {
			...
			if ($result) {
				if (mysql_num_rows($result) == 1) {
					...
					$data->emailbody = $member['emailbody'];
...
function sendJobMessage($jobid, $subject, $message, $attachments = array()) {
	$qry = "SELECT B.email, B.email2 AS clientemail2, B.firstname, A.email1, A.email2, A.email3 
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			...
			sendRoleMessage("NOTIFY", $subject, $message, $attachments);
